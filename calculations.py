    ...
    for document_name, data in documents.items():
        ...
        for word in document_words:
            if word in words:
                ...
                words[word] = 1
    return {word: math.log(total_documents / count) for word, count in words.items()}


def tf_idf_cosine_similarity(dict1, dict2, idf):
    all_keys = set(dict1.keys()).union(dict2.keys())

    vector1 = [dict1.get(key, 0) * idf[key] for key in all_keys]
    vector2 = [dict2.get(key, 0) * idf[key] for key in all_keys]
